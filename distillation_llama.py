...
def clean_label(label):
    # Entferne problematische Zeichen (z.B. doppelte Anführungszeichen)
...
@call_LLM is a function to send the prompt to the ollama LLM, which the user has chosen before
    ...
    teacher_text = teacher_tokenizer.decode(new_tokens, skip_special_tokens=True)
    #print("Generierter Text:", teacher_text)
    #teacher_text = teacher_tokenizer.decode(gen_ids[0], skip_special_tokens=True)

    # testen
    del inputs, teacher_out  # Falls diese nicht mehr benötigt werden
    torch.cuda.empty_cache()
    return teacher_text
...
def load_fsdp_teacher(model_name: str):
   ...
   bnb_4bit_compute_dtype=torch.float16
    ...
    #fsdp_model = FSDP(raw_model, cpu_offload=CPUOffload(offload_params=True))
...
for epoch in range(num_epochs):
    ...
    for iteration in range(amount_of_epk):
        ...
        data = nx.readwrite.json_graph.node_link_data(G, edges="links")
        ...
        # Falls es Sonderzeichen gibt, die vor dem speichern als JPEG entfernt werden sollten, passiert das hier. 
        # Evtl. Löschen
        for u, v, key, data in G.edges(keys=True, data=True):
            if 'label' in data:
                G[u][v][key]['label'] = clean_label(data['label'])
        ...
        try:
            ...
            print(f"Fehler beim Speichern des Graphens: {e}")
