...
amount_of_epk = 10 #int(input("Wie viele EPK's sollen generiert werden? "))
...
saving_directory = "/data/horse/ws/faku637g-specimen/llama_1_dist" #input("Zur Auswahl stehen: phi3, phi3more qwen, phi, mistral, llama3. Welches LLM Model soll verwendet werden? ")
...
'''
model_name = "/data/horse/ws/faku637g-specimen/student_models/llama_1B_sharding_3" #"meta-llama/Llama-3.3-70B-Instruct"
...
'''
    ...
    # Text generieren + Scores zurückgeben
    generation_output = model.generate(
        **inputs,
        ...
        output_logits=True,            
        do_sample=False,
        # weitere Parameter wie temperature, top_k, top_p etc. bei Bedarf
    )
    ...
    print("Generierter Text:", generated_text)
    #teacher_text = teacher_tokenizer.decode(gen_ids[0], skip_special_tokens=True)

    # Scores in ein Listenformat konvertieren (sonst nicht JSON-kompatibel)
    scores_as_lists = [score.cpu().tolist() for score in generation_output.logits]
    ...
    result = {
        ...
        "scores": scores_as_lists
    }

    # Hier extrahierst du noch den reinen Antwort-Text ohne „Input: ...“
    # (sofern du diesen Teil überhaupt brauchst)
    split_keyword = 'Input:'
    if split_keyword in result["decoded_text"]:
        answer = result["decoded_text"].split(split_keyword)[0].strip()
    else:
        answer = result["decoded_text"].strip()
    

    # Wenn du die kompletten Daten (inkl. Logits) zurückgeben willst, 
    # kannst du z. B. beides zurückgeben:
    del inputs
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    device_map=None, # test also with auto 
...
for iteration in range(amount_of_epk):
    ...
    '''
    placeholder = {
        "Temlate_Theme" : "Should be defined by user",
    ...
    file = f'/data/horse/ws/faku637g-specimen/Testdaten/epc_{iteration}.json' #'graphs/epc_11360__0.5169854086147916.json'
    ...
    for segment in data['segmentedGraph']:
        if segment['type'] == 'node':  
            ...
            if node_pattern_type == 'StartEPC':
                ...
                Output_StartEPC, logits = call_LLM(formatted_template_start_EPC)
                logits_list.append((node_id, logits))
                ...
                start_EPC = formatted_template_start_EPC
            ...
            if node_pattern_type == 'EndEPC':
                ...
                # Send the template to the LLM and get the result
                Output_EndEPC, logits = call_LLM(formatted_template)
                logits_list.append((node_id, logits))
                ...
                # Send the template to the LLM and get the result
                Output_ANDGate, logits = call_LLM(formatted_template)
                logits_list.append((node_id, logits))
                ...
                # Send the template to the LLM and get the result
                Output_ANDLOOP, logits = call_LLM(formatted_template)
                logits_list.append((node_id, logits))
                ...
                # Send the template to the LLM and get the result
                Output_ORGate, logits = call_LLM(formatted_template)
                logits_list.append((node_id, logits))
                ...
                # Send the template to the LLM and get the result
                Output_ORLOOP, logits = call_LLM(formatted_template)
                logits_list.append((node_id, logits))
                ...
                # Send the template to the LLM and get the result
                Output_XORGate, logits = call_LLM(formatted_template)
                logits_list.append((node_id, logits))
                ...
                # Send the template to the LLM and get the result
                Output_XORLOOP, logits = call_LLM(formatted_template)
                logits_list.append((node_id, logits))
        ...
        # Possible values are: XOR, OR, AND, LOOP
            ...
            if pattern_types_str == 'AND':
                ...
                Output_ANDEFE, logits = call_LLM(formatted_template)
                logits_list.append((node_id, logits))
            ...
            if pattern_types_str == 'XOR':
                ...
                Output_XOREFE, logits = call_LLM(formatted_template)
                logits_list.append((node_id, logits))
            ...
            if pattern_types_str == 'OR':
                ...
                # Send Template to LLM
                Output_OREFE, logits = call_LLM(formatted_template)
                logits_list.append((node_id, logits))
            ...
            if pattern_types_str == 'LOOP':
                ...
                # Send Template to LLM
                Output_LOOPEFE, logits = call_LLM(formatted_template)
                logits_list.append((node_id, logits))
    ...
    with open(f'{saving_directory}/ordered_templates_{iteration}.txt', "w") as text_file:
    ...
    with open(f'{saving_directory}/ordered_output_llm_{iteration}.txt', "w") as text_file:
        for order in output_list:
            text_file.write(f'{order[0]}: {order[1]}\n')

    with open(f'{saving_directory}/ordered_result_llm_{iteration}.txt', 'w', encoding='utf-8') as text_file:
        # ensure_ascii=False, damit Umlaute bzw. Unicode nicht escaped werden
        # indent=2 für eine übersichtliche Einrückung
        #json.dump(logits_list, f, ensure_ascii=False, indent=2)
        for order in logits_list:
            text_file.write(f'{logits[0]}: {logits[1]}\n')
            
    data_to_save = []
    #with open(f'{saving_directory}/ordered_result_llm_{iteration}.txt', 'w', encoding='utf-8') as text_file:
    #    for node_id, logits_data in logits_list:
    #    # logits_data ist hier ein Tensor bzw. eine Liste, je nachdem wie du es speicherst.
    #        text_file.write(f'{node_id}: {logits_data}\n')

    
    #for node_id, logits_data in logits_list:
    #    logits_as_lists = [ld.cpu().tolist() for ld in logits_data]
    #    data_to_save.append({
    #        "node_id": node_id,
    #        "logits": logits_as_lists  # oder ggf. .detach().cpu().tolist()
    #    })

    #with open(f'{saving_directory}/ordered_result_llm_{iteration}.json', 'w', encoding='utf-8') as f:
    #    json.dump(data_to_save, f, ensure_ascii=False, indent=2)

    with open(f"{saving_directory}/ordered_result_llm_{iteration}.txt", "w", encoding="utf-8") as text_file:
    # In EINER Schleife alles erledigen
        for node_id, logits_data in logits_list:
        # 1) In die TXT-Datei schreiben:
            text_file.write(f"{node_id}: {logits_data}\n")
            if isinstance(logits_data, torch.Tensor):
                logits_as_lists = logits_data.cpu().tolist()
            else:
                logits_as_lists = [ld.cpu().tolist() for ld in logits_data]

        data_to_save.append({
            "node_id": node_id,
            "logits": logits_as_lists
        })
    with open(f"{saving_directory}/ordered_result_llm_{iteration}.json", "w", encoding="utf-8") as f:
        json.dump(data_to_save, f, ensure_ascii=False, indent=2)
    ...
    jpeg_path = str(f'{saving_directory}/visualization_{iteration}.jpeg')
