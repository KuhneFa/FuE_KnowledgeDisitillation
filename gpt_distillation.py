...
amount_of_epk = 1 #int(input("Wie viele EPK's sollen generiert werden? "))
...
saving_directory = "hf_start_template" #input("Zur Auswahl stehen: phi3, phi3more qwen, phi, mistral, llama3. Welches LLM Model soll verwendet werden? ")
...
model_name = loss_distillation.teacher_model_name #"microsoft/Phi-3-mini-128k-instruct"
...
def call_LLM(teacher_model,teacher_tokenizer,student_model,student_tokenizer,optimizer,prompt):
    """
    1) Teacher generiert Text zu einem Prompt.
    2) Output wird tokenisiert.
    3) Student führt einen kurzen Training Step (1x forward/backward) aus.
    4) Gibt den generierten Text zurück (zur weiteren Verwendung).

    Note: 
    - Dies ist ein Minimalbeispiel! 
    - Für ernsthafte Knowledge Distillation brauchst du 
      z.B. logits vom Teacher, KL-Divergenz, Temperature-Scaling usw.
    """
    ########################################
    # 1) Teacher-Output generieren
    ########################################
    teacher_inputs = teacher_tokenizer(prompt, return_tensors="pt").to(device)
    with torch.no_grad():
        # Z.B. random sampling
    ...
    # (Optional) Print zur Kontrolle
    print(f"[Teacher Output]: {generated_text[:200]}...")

    ########################################
    # 2) Student-Trainingsschritt
    ########################################
    # a) Text -> Token
    #    Beispiel: wir nehmen den gesamten generierten Text als Input
    student_inputs = student_tokenizer(generated_text, return_tensors="pt").to(device)
    #    Labels = input_ids (typischer Language Modeling-Ansatz)
    ...
    # c) Loss berechnen (hier sehr vereinfacht)
    loss = simple_student_loss(student_logits, labels)

    # d) Backprop
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
    print(f"[Student Training] Loss: {loss.item():.4f}")

    # 3) Rückgabe des generierten Textes
...
for iteration in range(amount_of_epk):
    ...
    teacher_model_name = "gpt2"  # Beispiel
    ...
    student_model_name = "distilgpt2"  # Beispiel: kleineres Modell
    ...
    file = 'graphs/graph.json' #f'epc_simple/epc_{iteration}.json' #'graphs/epc_11360__0.5169854086147916.json'
