...
amount_of_epk = 50 #int(input("Wie viele EPK's sollen generiert werden? "))
...
saving_directory = "distill_sharding" #input("Zur Auswahl stehen: phi3, phi3more qwen, phi, mistral, llama3. Welches LLM Model soll verwendet werden? ")
...
# Vielleicht doch anmachen
os.environ["TOKENIZERS_PARALLELISM"] = "true"


def simple_student_loss(student_logits, labels):
    """
    Beispielhafter Loss für ein Language Model:
    - Hier nehmen wir an, dass `labels` = `input_ids` (Shifted LM Objective).
    - In einer richtigen KD-Schleife würdest du hier
      z.B. KL-Divergenz oder logit-matching gegen Teacher-Logits machen.
    """
...
def distillation_loss(
    ...
    # 2) (Optional) Hard-Label-Teil (CE)
    ce_loss = 0.0
    if hard_labels is not None and alpha_ce > 0.0:
        # shift für typisches LM-Training (optional, je nach Anwendungsfall)
        shift_student_logits = student_logits[..., :-1, :].contiguous()
        shift_labels = hard_labels[..., 1:].contiguous()
        ce_loss_fct = torch.nn.CrossEntropyLoss()
        ce_loss = ce_loss_fct(shift_student_logits.view(-1, shift_student_logits.size(-1)), 
                              shift_labels.view(-1))

    # Gesamtloss
    loss = alpha_ce * ce_loss + (1.0 - alpha_ce) * kd_loss
    return loss

def call_LLM(teacher_model,teacher_tokenizer,student_model,student_tokenizer,optimizer,prompt,temperature=2.0,alpha_ce=0.5):
    """
    1) Wir erstellen dieselbe Eingabesequenz `input_ids` für Teacher + Student,
       indem wir den Prompt tokenisieren.
    2) Teacher -> teacher_logits
    3) Student -> student_logits
    4) KD-Loss mit distillation_loss(...)
    5) Backprop ins Student-Modell.
    6) (Optional) Student-Output generieren oder Teacher-Output generieren, 
       wenn Sie den Text brauchen, um ihn z.B. zu labeln.
    """
    # 1) Prompt -> input_ids
    ...
    with torch.no_grad():
        ...
        teacher_logits = teacher_out.logits  # [batch_size, seq_len, vocab_size]
    ...
    loss = distillation_loss(
        ...
        alpha_ce=alpha_ce,
        hard_labels=input_ids if alpha_ce > 0 else None
    )
    ...
    print(f"[Distillation] KD-Loss: {loss.item():.4f}")

    # 6) Optional: generieren, wenn Sie den Text brauchen
    #    oder teacher_text = teacher_tokenizer.decode(...)
    #    oder student_text = ...
    #    Hier als Beispiel: Teacher generiert 1-2 Tokens weiter
    with torch.no_grad():
        gen_ids = teacher_model.generate(
            **inputs,
            max_new_tokens=100
        )
    teacher_text = teacher_tokenizer.decode(gen_ids[0], skip_special_tokens=True)
    return teacher_text

def init_distributed():
    """
    Initialisiert das verteilte Training für Single-Node Multi-GPU mit torchrun.
    """
...
# -------------------------------------------------
# HIER: Teacher-Modell als FSDP laden
# -------------------------------------------------
def load_fsdp_teacher(model_name: str):
    """
    Lädt ein großes Teacher-Modell (z.B. 70B) in 8-Bit und wrappt es in FSDP.
    Ohne `device_map="auto"`, damit wir `.to(device)` + FSDP nutzen können.
    """
    # Konfiguration für 8-Bit-Quantisierung via bitsandbytes
   ...
   bnb_4bit_compute_dtype=torch.float16
    ...
    raw_model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map=None,        # Deaktiviert Accelerate-Hooks
        trust_remote_code=True, # falls Llama etc.
    )

    # Manuelle FSDP-Wrapping
    fsdp_model = wrap(raw_model)

    # Bestimmen local_rank
    local_rank = int(os.environ["LOCAL_RANK"])
    device = torch.device(f"cuda:{local_rank}")

    # Jetzt können wir das Modell manuell verschieben
    fsdp_model = fsdp_model.to(device)
    fsdp_model.eval()

    return fsdp_model, tokenizer

def load_fsdp_student(model_name: str):
    """
    Lädt ein großes Teacher-Modell (z.B. 70B) in 8-Bit und wrappt es in FSDP.
    Ohne `device_map="auto"`, damit wir `.to(device)` + FSDP nutzen können.
    """
...
teacher_model_name = "gpt2" #"meta-llama/Llama-3.2-1B-Instruct" #tiiuae/Falcon3-10B-Instruct #"deepseek-ai/DeepSeek-V3"  # Beispiel
teacher_model, teacher_tokenizer = load_fsdp_teacher(teacher_model_name)

# 2) Student initialisieren
student_model_name =   "gpt2" #"meta-llama/Llama-3.2-1B-Instruct"  #"microsoft/phi-4" Beispiel: kleineres Modell
...
#student_model = wrap(student_model)
...
for epoch in range(num_epochs):
    ...
    for iteration in range(amount_of_epk):
        ...
        file = f'/data/horse/ws/faku637g-specimen/Trainingsdaten/epc_{iteration}.json' 
        ...
        for segment in data['segmentedGraph']:
            if segment['type'] == 'node':  
                ...
                if node_pattern_type == 'StartEPC':
                    ...
                    Output_StartEPC = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
                prompt=formatted_template_start_EPC)
                ...
                if node_pattern_type == 'EndEPC':
                    ...
                    #formatted_template = f"User: {formatted_template}"
                    #formatted_template = f"{start_EPC} User: {formatted_template}"

                    # Send the template to the LLM and get the result
                    Output_EndEPC = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
                prompt=formatted_template)
                    ...
                    # Send the template to the LLM and get the result
                    Output_ANDGate = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
                prompt=formatted_template)
                    ...
                    # Send the template to the LLM and get the result
                    Output_ANDLOOP = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
                prompt=formatted_template)
                    ...
                    # Send the template to the LLM and get the result
                    Output_ORGate = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
                prompt=formatted_template)
                    ...
                    # Send the template to the LLM and get the result
                    Output_ORLOOP = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
                prompt=formatted_template)
                    ...
                    # Send the template to the LLM and get the result
                    Output_XORGate = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
                prompt=formatted_template)
                    ...
                    # Send the template to the LLM and get the result
                    Output_XORLOOP = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
            ...
            # Possible values are: XOR, OR, AND, LOOP
                ...
                if pattern_types_str == 'AND':
                    ...
                    Output_ANDEFE = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
                ...
                if pattern_types_str == 'XOR':
                    ...
                    Output_XOREFE = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
                ...
                if pattern_types_str == 'OR':
                    ...
                    # Send Template to LLM
                    Output_OREFE = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
                ...
                if pattern_types_str == 'LOOP':
                    ...
                    # Send Template to LLM
                    Output_LOOPEFE = call_LLM(teacher_model=teacher_model,
                teacher_tokenizer=teacher_tokenizer,
                student_model=student_model,
                student_tokenizer=student_tokenizer,
                optimizer=optimizer,
        ...
        with open(f'{saving_directory}/ordered_templates_{iteration}_{num_epochs}.txt', "w") as text_file:
        ...
        with open(f'{saving_directory}/ordered_output_llm_{iteration}_{num_epochs}.txt', "w") as text_file:
        ...
        output_file = f'{saving_directory}/modified_graph_{iteration}.json'
        ...
        jpeg_path = str(f'{saving_directory}/visualization_{iteration}_{num_epochs}.jpeg')
        pydot_graph.write_png(jpeg_path)

        # Calculate the time needed to create the epc
        elapsed_time = time.time() - start_time
        student_model.save_pretrained(f"/data/horse/ws/faku637g-specimen/student_models/both_sharding_{num_epochs}")
        teacher_model.save_pretrained(f"/data/horse/ws/faku637g-specimen/models_cache/teacher_{num_epochs}")
...
#dist.barrier()
if rank == 0:
    print("Training / Distillation fertig.")
