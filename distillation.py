...
def distillation_loss(
    ...
    """
    KL-Divergenz-basierter KD-Loss zwischen Teacher- und Student-Logits:
    - Optional: Mischen mit Hard-Label-CE, wenn Hard-Labels vorhanden (alpha_ce > 0)
    - temperature: Skaliert Logits von Teacher+Student
    - alpha_ce: Mischverhältnis. alpha_ce=0 => Nur Distillation. alpha_ce=1 => Nur Hard-Label-Loss

    Args:
        student_logits: [batch_size, seq_len, vocab_size]
        teacher_logits: [batch_size, seq_len, vocab_size]
        temperature (float): z.B. 2.0
        alpha_ce (float): 0..1, Gewichtung des Hard-Label-Anteils
        hard_labels (Optional[Tensor]): Falls Hard-Labels existieren (Größe [batch_size, seq_len])

    Returns:
        loss (torch.Tensor): Skalar, der Distillationsverlust
    """
    # 1) KL-Divergenz-Teil
    #   teacher_probs = Softmax(TeacherLogits / T)
    #   student_log_probs = logSoftmax(StudentLogits / T)
    student_log_probs = F.log_softmax(student_logits / temperature, dim=-1)
    teacher_probs = F.softmax(teacher_logits / temperature, dim=-1)

    # KL-Divergenz: KL(teacher_probs || student_log_probs)
    # => F.kl_div(..., reduction="batchmean") rechnet \sum teacher_probs * log(teacher_probs / student_probs).
...
student_model.save_pretrained("/data/horse/ws/faku637g-specimen/models_cache/student")
teacher_model.save_pretrained("/data/horse/ws/faku637g-specimen/models_cache/teacher")
